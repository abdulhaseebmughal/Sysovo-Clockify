...
export default function TrelloBoard() {
 ...
 const fetchBoards = async () => {
  ...
  const handleUpdateBoard = async () => {
    ...
    try {
      const res = await api.put(`/api/boards/${editingBoard._id}`, {
        name: editingBoard.name,
        description: editingBoard.description
      });
      const updated = res.data.board || res.data;
      setBoards((p) => p.map((b) => (b._id === updated._id ? updated : b)));
      if (selectedBoard?._id === updated._id) setSelectedBoard(updated);
      ...
      <div style={{
        ...
        <div style={{ maxHeight: "calc(100vh - 280px)", overflowY: "auto", overflowX: "hidden" }}>
          ...
          {boards.map((b) => (
            ...
            >
              <div onClick={() => selectBoard(b)} style={{ paddingRight: 60 }}>
                <div style={{
                  fontWeight: 600,
                  fontSize: 14,
                  marginBottom: 3,
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{
                    background: selectedBoard?._id === b._id ? "#CCFF00" : "rgba(204, 255, 0, 0.8)",
                  color: "var(--bg-main)",
                  border: "none",
                  borderRadius: 4,
                  padding: "4px 8px",
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = selectedBoard?._id === b._id ? "#CCFF00" : "rgba(204, 255, 0, 0.8)";
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                style={{
                  padding: "10px 20px",
                  background: editingBoard.name?.trim() ? "#CCFF00" : "var(--border)",
                  color: editingBoard.name?.trim() ? "var(--bg-main)" : "var(--text-secondary)",
                  border: "none",
                  borderRadius: 6,
                  cursor: editingBoard.name?.trim() ? "pointer" : "not-allowed",
                  fontWeight: 600,
                  fontSize: 14,
                  transition: "all 0.2s ease",
                  opacity: editingBoard.name?.trim() ? 1 : 0.5
                }}
                onMouseEnter={(e) => {
